...
void PowerMes::powerOUToff() {
  pinMode(Vext, OUTPUT);
  digitalWrite(Vext, HIGH); // Vext OFF (High Impedance / High)
  disableAllGroups();
}
...
void PowerMes::powerOUTon() {
  ...
  // Enable ALL groups (Legacy behavior)
  enableGroupA();
  enableGroupB();
  enableGroupC();
...
void PowerMes::enableGroupA() {
  // Group A: Wind Speed (GPIO10)
  // Shared with OLED REST. If OLED is active, DO NOT toggle this pin!
#if DEBUG_OLED
  DEBUG_PRINTLN(F("[PWR] Group A SKIPPED (OLED Active)"));
#else
  pinMode(PIN_POWER_GRP_A, OUTPUT);
  digitalWrite(PIN_POWER_GRP_A, HIGH);
  DEBUG_PRINTLN(F("[PWR] Group A ON"));
#endif
}

void PowerMes::enableGroupB() {
  // Group B: Wind Direction (GPIO4)
  pinMode(PIN_POWER_GRP_B, OUTPUT);
  digitalWrite(PIN_POWER_GRP_B, HIGH);
  DEBUG_PRINTLN(F("[PWR] Group B ON"));
}

void PowerMes::enableGroupC() {
  // Group C: Environment (GPIO7)
  pinMode(PIN_POWER_GRP_C, OUTPUT);
  digitalWrite(PIN_POWER_GRP_C, HIGH);
  DEBUG_PRINTLN(F("[PWR] Group C ON"));
}

void PowerMes::disableAllGroups() {
  // Turn OFF all Group Pins
  // Group A
#if !DEBUG_OLED
  digitalWrite(PIN_POWER_GRP_A, LOW);
  pinMode(PIN_POWER_GRP_A, INPUT); // High Z
#endif

  // Group B
  digitalWrite(PIN_POWER_GRP_B, LOW);
  pinMode(PIN_POWER_GRP_B, INPUT);

  // Group C
  digitalWrite(PIN_POWER_GRP_C, LOW);
  pinMode(PIN_POWER_GRP_C, INPUT);

  DEBUG_PRINTLN(F("[PWR] All Groups OFF"));
}
