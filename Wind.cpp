...
Wind::Wind()
    : _encoder(nullptr), _windDIR(N), _currentDegrees(0), _northOffset(0),
      _initialized(false) {}
...
void Wind::debugStatus(uint8_t status) {
  ...
  DEBUG_PRINT(F("Raw Status Byte: 0x"));
  DEBUG_PRINT(status, HEX);
  DEBUG_PRINT(F(" (Binary: "));
  DEBUG_PRINT(status, BIN);
  DEBUG_PRINTLN(F(")"));

  // Bit 5: MD (Magnet Detected)
  if (status & 0x20) {
    DEBUG_PRINTLN(F("✓ MD (bit 5): Magnet DETECTED"));
  } else {
    DEBUG_PRINTLN(
        F("✗ MD (bit 5): Magnet NOT detected - Check magnet presence!"));
  }

  // Bit 4: ML (Magnet too Low/Weak)
  if (status & 0x10) {
    DEBUG_PRINTLN(F("⚠ ML (bit 4): Magnet TOO WEAK - Move magnet closer!"));
  } else {
    DEBUG_PRINTLN(F("✓ ML (bit 4): Magnet signal OK"));
  }

  // Bit 3: MH (Magnet too High/Strong)
  if (status & 0x08) {
    DEBUG_PRINTLN(F("⚠ MH (bit 3): Magnet TOO STRONG - Move magnet away!"));
  } else {
    DEBUG_PRINTLN(F("✓ MH (bit 3): Magnet distance OK"));
  }
  DEBUG_PRINTLN(F("-----------------------------------\n"));
}
...
bool Wind::init() {
  ...
  DEBUG_PRINTLN(F("[WIND] Calling encoder.begin()..."));
  _encoder->begin();

  // Verifica connessione I2C
  if (!_encoder->isConnected()) {
    DEBUG_PRINTLN(F("[WIND] ✗ FAILED: AS5600 not responding on I2C"));
    DEBUG_PRINTLN(F(" -> Check SDA/SCL wiring & Pullups"));
    DEBUG_PRINTLN(F(" -> Check I2C address (should be 0x36)"));
    return false;
  }

  DEBUG_PRINTLN(F("[WIND] ✓ AS5600 found on I2C bus"));
  ...
  if (!(status & 0x20)) {
    DEBUG_PRINTLN(F("[WIND] ✗ CRITICAL: Magnet NOT detected. Init failed."));
  ...
  if ((status & 0x10) || (status & 0x08)) {
    DEBUG_PRINTLN(
        F("[WIND] ⚠ WARNING: Magnet distance not optimal (see above)"));
  } else {
    DEBUG_PRINTLN(F("[WIND] ✓ Magnet alignment OPTIMAL"));
  }

  _initialized = true;
  DEBUG_PRINTLN(F("[WIND] Initialization COMPLETE\n"));
  return true;
}

float Wind::getSpeed() {
  // Lettura ADC per Anemometro (Group A)
  // Ritorna il valore raw o convertito. Per ora ritorniamo mV o Raw.
  // AnalogRead su ESP32 ritorna 0-4095.
  uint16_t raw = analogRead(ADC_WIND_S);
  return (float)raw; // TODO: Calibrazione km/h
}

void Wind::update() {
  if (!_initialized) {
    DEBUG_PRINTLN(F("[WIND] Skipped update: Not Initialized"));
    return;
  }
...
const char *Wind::directionToString() const {
  return WIND_DIR_STRINGS[_windDIR];
}
