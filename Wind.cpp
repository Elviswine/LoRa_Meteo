...
void Wind::debugStatus(uint8_t status) {
  ...
  Serial.print("Raw Status Byte: 0x");
  ...
  if (status & 0x20) {
    ...
    Serial.println(
        "✗ MD (bit 5): Magnet NOT detected - Check magnet presence!");
  }
...
bool Wind::init() {
  ...
  DEBUG_PRINTLN("[WIND] Calling encoder.begin()...");
  _encoder->begin();

  // Verifica connessione I2C
  if (!_encoder->isConnected()) {
    DEBUG_PRINTLN("[WIND] ✗ FAILED: AS5600 not responding on I2C");
    DEBUG_PRINTLN(" -> Check SDA/SCL wiring & Pullups");
    DEBUG_PRINTLN(" -> Check I2C address (should be 0x36)");
    return false;
  }

  DEBUG_PRINTLN("[WIND] ✓ AS5600 found on I2C bus");
  ...
  if (!(status & 0x20)) {
    DEBUG_PRINTLN("[WIND] ✗ CRITICAL: Magnet NOT detected. Init failed.");
    return false; // Blocchiamo l'init se non c'è il magnete
  }

  // Controllo distanza (Warning ma non bloccante)
  if ((status & 0x10) || (status & 0x08)) {
    DEBUG_PRINTLN("[WIND] ⚠ WARNING: Magnet distance not optimal (see above)");
  } else {
    DEBUG_PRINTLN("[WIND] ✓ Magnet alignment OPTIMAL");
  }

  _initialized = true;
  DEBUG_PRINTLN("[WIND] Initialization COMPLETE\n");
  return true;
}

void Wind::update() {
  if (!_initialized) {
    DEBUG_PRINTLN("[WIND] Skipped update: Not Initialized");
    return;
  }
