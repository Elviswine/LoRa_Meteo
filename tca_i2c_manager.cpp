...
void TcaI2cManager::setWire(TwoWire &w) {
    _wire = &w;
}

bool TcaI2cManager::isInitialized() const {
    return _tca_initialized;
}
...
bool TcaI2cManager::detectTcaAddress() {
    ...
    for (uint8_t addr = 0x70; addr <= 0x77; addr++) {
        ...
        if (_wire->endTransmission() == 0) {
            _tca_addr = addr;
            Serial.printf("[TCA] Found TCA9548A at 0x%02X on bus %p\n", addr, (void*)_wire);
            return true;
        }
    }
    return false;
...
bool TcaI2cManager::selectChannel(uint8_t ch) {
    if (ch >= TCA_NUM_CHANNELS) return false;
    _wire->beginTransmission(_tca_addr);
    _wire->write(1 << ch);
    if (_wire->endTransmission() != 0) {
        Serial.printf("[TCA] ERROR: selectChannel(%u) failed on addr 0x%02X\n", ch, _tca_addr);
        return false;
    }
    delay(5);
    return true;
...
void TcaI2cManager::initAsync() {
    ...
    if (_wire == nullptr) {
        Serial.println("[TCA] ERROR: Wire not set!");
        return;
    }

    // Trova TCA (scan 0x70..0x77)
    if (!detectTcaAddress()) {
        Serial.println("[TCA] ERROR: TCA not found (0x70..0x77)");
        return;
    }
    ...
    for (uint8_t ch = 0; ch < TCA_NUM_CHANNELS; ch++) {
        if (TCA_CH_TYPE[ch] == SENS_NONE) continue;
        ...
        Serial.printf("[TCA] CH%u: Found device @ 0x%02X (type=%s)\n",
                      ...
                      (TCA_CH_TYPE[ch] == SENS_SHT3X ? "SHT3X" :
                       TCA_CH_TYPE[ch] == SENS_SHT4X ? "SHT4X" : "BME280"));
        ...
        // ====================================================================
            ...
            bool ok = false;
            if (sht4x_ptrs[ch] != nullptr) {
                ok = sht4x_ptrs[ch]->begin();  // SHT4x non supporta TwoWire nel costruttore
    ...
    Serial.println("[TCA] initAsync completed.");
}
...
void TcaI2cManager::read() {
    if (!_tca_initialized) {
        Serial.println("[TCA] read() called but not initialized.");
        return;
    }

    for (uint8_t ch = 0; ch < TCA_NUM_CHANNELS; ch++) {
        if (!_channel_online[ch]) continue;
...
void TcaI2cManager::readSHT3X(uint8_t ch) {
    if (sht31_ptrs[ch] == nullptr) return;
    if (!selectChannel(ch)) {
...
void TcaI2cManager::readSHT4X(uint8_t ch) {
    if (sht4x_ptrs[ch] == nullptr) return;
    if (!selectChannel(ch)) {
...
void TcaI2cManager::readBME280(uint8_t ch) {
    if (bme280_ptrs[ch] == nullptr) return;
    if (!selectChannel(ch)) {
    ...
    for (uint8_t ch = 0; ch < TCA_NUM_CHANNELS; ch++) {
        ...
        if (TCA_CH_TYPE[ch] == SENS_BME280 && bme280_online[ch]) {
            Serial.printf(" CH%u [BME280] T=%.2fC P=%.1fhPa H=%.1f%%\n", ch,
                         bme280_temperature[ch], bme280_pressure[ch], bme280_humidity[ch]);
        } else if (TCA_CH_TYPE[ch] == SENS_BME280) {
